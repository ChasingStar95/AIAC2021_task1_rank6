...
from modules import CNN_BLOCK, Attention2, SpatialDropout
...
SAVE_PATH = '../final_output/enhancedrcnn1001'
...
class PointClfModel(nn.Module):
    def __init__(self, emb_dim, head_num, attention_layers):
        ...
        gru_size = 1280
        conv_size = 256
        self.Gru = nn.GRU(W2V_DIM, gru_size // 2, bidirectional=True, batch_first=True)
        self.video_linear = nn.Linear(dataset.video_shape[1], W2V_DIM)
        # self.QueryGru = nn.GRU(EMB_DIM, GRU_SIZE // 2, bidirectional=True, batch_first=True)
        # self.TitleGru = nn.GRU(EMB_DIM, GRU_SIZE // 2, bidirectional=True, batch_first=True)
        self.TextConv = CNN_BLOCK(in_channels=gru_size, out_channels=conv_size, relu=False, norm=True)
        self.VideoConv = CNN_BLOCK(in_channels=gru_size, out_channels=conv_size, relu=False, norm=True)

        self.TextAttention = Attention2(feature_dim=gru_size, step_dim=32)
        self.VideoAttention = Attention2(feature_dim=gru_size, step_dim=32)

        DENSE_SIZE = (conv_size * 2 + gru_size * 8) * 4
        self.encode_linear = nn.Linear(DENSE_SIZE, emb_dim)
    ...
    def encoder(self, title_seq, video_matrix):
        ...
        title_emb = self.embedding_dropout(title_emb)
        title_gru, _ = self.Gru(title_emb)
        title_conv = self.TextConv(title_gru)  # .permute(0, 2, 1)
        t_conv_avg_pool = torch.mean(title_conv, 1)
        t_conv_max_pool, _ = torch.max(title_conv, 1)

        video_emb = self.video_linear(video_matrix)
        video_emb = self.embedding_dropout(video_emb)
        video_gru, _ = self.Gru(video_emb)
        video_conv = self.VideoConv(video_gru)  # .permute(0, 2, 1)
        v_conv_avg_pool = torch.mean(video_conv, 1)
        v_conv_max_pool, _ = torch.max(video_conv, 1)

        t_soft_att, v_soft_att = self.soft_align(title_gru, video_gru)

        t_att = self.TextAttention(title_gru)
        t_att_diff = t_soft_att - t_att
        t_att_angle = t_soft_att * t_att
        t_att_total = torch.cat((t_soft_att, t_att, t_att_diff, t_att_angle), 2)
        t_att_avg_pool = torch.mean(t_att_total, 1)
        t_att_max_pool, _ = torch.max(t_att_total, 1)

        v_att = self.VideoAttention(video_gru)
        v_att_diff = v_soft_att - v_att
        v_att_angle = v_soft_att * v_att
        v_att_total = torch.cat((v_soft_att, v_att, v_att_diff, v_att_angle), 2)
        v_att_avg_pool = torch.mean(v_att_total, 1)
        v_att_max_pool, _ = torch.max(v_att_total, 1)

        title_concat = torch.cat((t_conv_avg_pool, t_conv_max_pool, t_att_avg_pool, t_att_max_pool), dim=1)
        video_concat = torch.cat((v_conv_avg_pool, v_conv_max_pool, v_att_avg_pool, v_att_max_pool), dim=1)

        tv_diff = title_concat - video_concat
        tv_angle = title_concat * video_concat
        concat = torch.cat((title_concat, video_concat, tv_diff, tv_angle), 1)
...
if DISTILL_TRAIN2:
    ...
    for _fold, (train_idx, valid_idx) in enumerate(kf.split(label_df, label_df)):
        ...
        lr = even_mults(LR / 2, LR, 4)

        optimizer_parameters = [
            {'params': model.module.pointwise_module.module.WordEmb.parameters(), 'lr': lr[0]}, # TextConv
            {'params': model.module.pointwise_module.module.video_linear.parameters(), 'lr': lr[0]},

            {'params': model.module.pointwise_module.module.Gru.parameters(), 'lr': lr[1]},

            {'params': model.module.pointwise_module.module.TextConv.parameters(), 'lr': lr[2]},
            {'params': model.module.pointwise_module.module.VideoConv.parameters(), 'lr': lr[2]},
            {'params': model.module.pointwise_module.module.TextAttention.parameters(), 'lr': lr[2]},
            {'params': model.module.pointwise_module.module.VideoAttention.parameters(), 'lr': lr[2]},

            {'params': model.module.pointwise_module.module.encode_linear.parameters(), 'lr': lr[3]}
        ]
