...
from modules import SpatialDropout, NeXtVLAD
...
SAVE_PATH = '../final_output/lstm_nextvlad_siamese1001'
...
class PointClfModel(nn.Module):
    def __init__(self, emb_dim):
        ...
        encode_size = 512
        self.LSTM = nn.LSTM(W2V_DIM, encode_size // 2, bidirectional=True, batch_first=True)

        video_emb_size = dataset.video_shape[1]
        self.video_nextvlad = NeXtVLAD(feature_size=video_emb_size, output_size=encode_size, expansion_size=2, cluster_size=64, groups=8)
        self.encode_linear = nn.Linear(encode_size*4, emb_dim)
    ...
    def encoder(self, title_seq, video_matrix):
        ...
        title_emb = torch.mean(title_lstm, 1)

        video_emb = self.video_nextvlad(video_matrix)

        concat_emb = torch.cat((title_emb, video_emb, title_emb - video_emb, title_emb * video_emb), 1)
        encoded_output = self.encode_linear(concat_emb)
...
if POINTWISE_TRAIN:
    ...
    model = PointClfModel(ENCODE_DIM)
    model.to(device)
...
if PAIRWISE_TRAIN:
    ...
    for _fold, (train_idx, valid_idx) in enumerate(kf.split(dataset.label_df, dataset.label_df)):
        ...
        torch.cuda.empty_cache()
        device = torch.device("cuda:0")
        point_model = PointClfModel(emb_dim=ENCODE_DIM)
...
if DISTILL_TRAIN2:
    ...
    for _fold, (train_idx, valid_idx) in enumerate(kf.split(label_df, label_df)):
        ...
        lr = even_mults(LR / 2, LR, 3)

        optimizer_parameters = [
            {'params': model.module.pointwise_module.module.WordEmb.parameters(), 'lr': lr[0]},

            {'params': model.module.pointwise_module.module.LSTM.parameters(), 'lr': lr[1]},
            {'params': model.module.pointwise_module.module.video_nextvlad.parameters(), 'lr': lr[1]},

            {'params': model.module.pointwise_module.module.encode_linear.parameters(), 'lr': lr[2]}
        ]
