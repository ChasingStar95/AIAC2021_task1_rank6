...
SAVE_PATH = '../final_output/lstm1001'
...
class PointClfModel(nn.Module):
    def __init__(self, emb_dim):
        ...
        # text_size = 1024
        # self.title_linear = nn.Linear(W2V_DIM, fit_size)
        # self.video_linear = nn.Linear(dataset.video_shape[1], fit_size)

        lstm_size = 1024
        self.Lstm = nn.LSTM(W2V_DIM+dataset.video_shape[1], lstm_size // 2, bidirectional=True, batch_first=True)
        self.video_linear = nn.Linear(dataset.video_shape[1], W2V_DIM)

        self.encode_linear = nn.Linear(lstm_size*2, emb_dim)
    ...
    def encoder(self, title_seq, video_matrix):
        ...
        concat_emb = torch.cat((title_emb, video_matrix), -1)
        concat_emb = self.embedding_dropout(concat_emb)

        concat_lstm, _ = self.Lstm(concat_emb)

        concat_avg_pool = torch.mean(concat_lstm, 1)
        concat_max_pool, _ = torch.max(concat_lstm, 1)
        concat = torch.cat((concat_avg_pool, concat_max_pool), 1)
