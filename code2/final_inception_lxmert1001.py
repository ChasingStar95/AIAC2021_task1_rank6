...
from modules import LXRTXLayer, SpatialDropout, Inception
...
SAVE_PATH = '../final_output/inception_lxmert1001'
...
class PointClfModel(nn.Module):
    def __init__(self, emb_dim, config):
        ...
        encode_size = config.hidden_size
        self.text_inception1 = Inception(W2V_DIM, encode_size // 2)
        self.text_inception2 = Inception(encode_size // 2, encode_size // 2)
        self.video_inception1 = Inception(dataset.video_shape[1], encode_size // 2)
        self.video_inception2 = Inception(encode_size // 2, encode_size // 2)
    ...
    def encoder(self, title_seq, video_matrix):
        ...
        text_encode1 = self.text_inception1(text_encode.permute(0, 2, 1))
        text_encode2 = self.text_inception2(text_encode1)
        text_encode1 = text_encode1.permute(0, 2, 1)
        text_encode2 = text_encode2.permute(0, 2, 1)
        text_encode = torch.cat((text_encode1, text_encode2), 2)

        # video_encode = self.video_linear(video_matrix)
        video_encode = self.embedding_dropout(video_matrix)
        video_encode1 = self.video_inception1(video_encode.permute(0, 2, 1))
        video_encode2 = self.video_inception2(video_encode1)
        video_encode1 = video_encode1.permute(0, 2, 1)
        video_encode2 = video_encode2.permute(0, 2, 1)
        video_encode = torch.cat((video_encode1, video_encode2), 2)
